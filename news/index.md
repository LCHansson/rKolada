...
- **[`values_legend()`](https://lchansson.github.io/rKolada/reference/values_legend.md)
  ...
  the same API in the sibling package `rTrafa`. `lang` (default `"SV"`,
  settable via `getOption("rKolada.lang", "SV")`) toggles the source
  prefix between `"Källa: Kolada"` and `"Source: Kolada"`.
  `omit_varname` drops the `"N01951: "` KPI code prefix; `omit_desc`
  shows only the codes.
- **[`values_legend()`](https://lchansson.github.io/rKolada/reference/values_legend.md)’s
  `kpi_df` argument is now optional.** When omitted, the legend falls
  back to a codes-only listing.
- **Source prefix in plot captions**:
  [`values_legend()`](https://lchansson.github.io/rKolada/reference/values_legend.md)
  now prepends a `"Källa: Kolada"` / `"Source: Kolada"` line to the
  caption by default. This is a minor change to the rendered caption for
  existing users.
...
- Vignette plots now convert `year` to `Date` before plotting and use
  ...
  land on whole years rather than on decimal years like
  `2020, 2022.5, 2025`. This pattern is explained inline in both
  vignettes and is consistent across the sibling packages `pixieweb` and
  `rTrafa`.
